...
export default function VerifiedReportsPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-12">
      ...
      <div className="p-6 rounded-2xl border border-emerald-500/40 bg-emerald-500/[0.04] backdrop-blur-xl space-y-3">
        ...
        <div className="flex items-center gap-4 text-xs text-white/50 pt-1 font-mono">
      ...
      <div className="rounded-2xl border border-white/10 bg-[#070D0A]/95 p-6 backdrop-blur-xl space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-white/10">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono text-white/40">({filtered.length} records)</span>
          </div>
          ...
          <div className="flex items-center gap-1.5 p-1 rounded-xl bg-white/5 border border-white/10 text-xs font-mono">
            <button
              ...
              className={`px-3 py-1 rounded-lg transition-all ${
                filter === "all" ? "bg-teal-500 text-black font-bold" : "text-white/60 hover:text-white"
              }`}
            >
              All
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-lg transition-all ${
                filter === "action_scheduled" ? "bg-teal-500 text-black font-bold" : "text-white/60 hover:text-white"
              }`}
            >
              Action Scheduled
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-lg transition-all ${
                filter === "resolved" ? "bg-teal-500 text-black font-bold" : "text-white/60 hover:text-white"
              }`}
            >
              Resolved
            </button>
        ...
        <div className="divide-y divide-white/5">
          {filtered.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="py-4 flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs group hover:bg-white/[0.01] px-2 rounded-xl transition-all"
              >
                <div className="space-y-1.5 flex-1">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="font-semibold text-white/90">{areaName}</span>
                    <span className="text-white/40 font-mono">({item.reportCount} reports)</span>
                    <StatusBadge
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <Link href={`/supervisor/water/cases/${item.id}`}>
                    <Button
                      variant="outline"
                      size="sm"
                      className="border-white/10 bg-white/5 text-xs text-teal-400 hover:bg-teal-500/10 hover:border-teal-500/30 gap-1.5 h-8"
                    >
