    ...
    <div className="space-y-6">
        ...
        actions={
          <div className="flex items-center gap-2">
            <Link href="/gov/alerts?type=power">
              <Button variant="outline" size="sm" className="h-8 gap-1.5 border-border bg-muted text-xs text-amber-ink">
      ...
      {creating && (
        <form onSubmit={handleCreateDr} className="p-6 rounded-2xl border border-amber-500/30 bg-amber-500/[0.04] space-y-4">
          <h3 className="text-sm font-bold text-foreground">Dispatch Automated Demand Response (ADR) Event</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-soft mb-1">Target Reduction (MW)</label>
              <input
                type="number"
                value={targetMw}
            ...
            <div>
              <label className="block text-xs font-medium text-soft mb-1">Time Window</label>
              <input
                type="text"
                value={windowStr}
          ...
          <div className="flex justify-end gap-2">
            <Button type="button" variant="ghost" size="sm" onClick={() => setCreating(false)} className="text-xs text-muted-foreground">
              Cancel
            </Button>
            <Button type="submit" size="sm" className="bg-primary text-primary-foreground hover:bg-primary-hover text-xs font-bold">
              Dispatch ADR Signal
            </Button>
          </div>
        </form>
      ...
      <div className="rounded-2xl border border-border bg-card p-6 backdrop-blur-xl">
        <h3 className="text-sm font-bold text-foreground mb-4">Active & Scheduled Demand Response Events</h3>
            ...
            <div key={ev.id} className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-bold text-foreground font-mono">{ev.id}</span>
                  <StatusBadge status={ev.status === "active" ? "warning" : "complete"} label={ev.status.toUpperCase()} />
                </div>
                <p className="text-muted-foreground">
                  Window: <span className="font-mono text-foreground">{ev.windowStart ? `${ev.windowStart.slice(11, 16)}–${ev.windowEnd.slice(11, 16)} UTC` : "Peak Period"}</span> · Target: <span className="font-mono text-amber-ink font-bold">{ev.targetMw} MW</span> · Scope: {ev.wardIds.length ? ev.wardIds.join(", ") : "City-wide"}
                </p>
              </div>

              <div className="flex items-center gap-4 font-mono">
                <div>
                  <span className="text-2xs text-faint block">OPTED IN</span>
                  <span className="text-foreground font-bold">{ev.optedInHouseholds.toLocaleString()} homes</span>
                </div>
                <div>
                  <span className="text-2xs text-faint block">AVERTED</span>
                  <span className="text-positive font-bold">{ev.avertedMw.toFixed(1)} MW</span>
                </div>
              </div>
            </div>
          ))}
